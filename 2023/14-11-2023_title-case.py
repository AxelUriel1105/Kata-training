...
Arguments (Haskell)
First argument: space-delimited list of minor words that must always be lowercase except for the first word in
 the string.
...
for the first word in the string. The JavaScript/CoffeeScript tests will pass undefined when this argument
 is unused.
Example
...
def title_case(title, minor_words=''):
    ...
    return " ".join(i.lower().capitalize() if i.lower() not in minor_words or j==0 else i.lower() for j,i in enumerate(title.split()))
