...
def bits_battle(numbers):
    odds = sum(bin(ones)[2::].count('1') for ones in numbers if ones%2!=0 and ones is not 0)
    even = sum(bin(zeros)[2::].count('0') for zeros in numbers if zeros%2==0 and zeros is not 0)
    print (odds, even)
    return {
        odds > even: 'odds win',
        odds < even: 'evens win',
