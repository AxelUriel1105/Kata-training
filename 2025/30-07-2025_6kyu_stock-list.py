...
def stock_list(stocklist, categories):
    books = [(b.split()[0],b.split()[1]) for b in stocklist]
    categories_dict = dict(zip(categories,[0]*len(categories)))
    for cat,n in books:
        if cat[0] in categories_dict:
            categories_dict[cat[0]]+=int(n)
        
    return ' - '.join(f'({k} : {categories_dict[k]})'for k in categories_dict) if stocklist else ''