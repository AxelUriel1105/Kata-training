...
def test(r):
    did_well = ['They did well'] if all(n>=9 for n in r) else []
    results = [{'h':sum((r.count(9),r.count(10))),'a':sum((r.count(7),r.count(8))),'l':len(list(filter(lambda x: x<=6 and x>=1,r)))}]
    return [round(sum(r)/len(r),3)] + results + did_well