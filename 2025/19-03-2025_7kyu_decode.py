...
import math, string
def decode(code, key):
    key = math.ceil(len(code)/len(str(key)))*str(key)
    alphabet_dict = dict(zip(range(1,27),string.ascii_lowercase))
    return ''.join(alphabet_dict.get(c-int(k)) for c,k in zip(code, key))
