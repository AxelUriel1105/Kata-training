...
def diagonal(matrix):
    ...
    secondary_diagonal_sum = sum(arr[i] for arr,i in zip(matrix,range(len(matrix)-1,-1,-1)))
    return {
        principal_diagonal_sum>secondary_diagonal_sum: "Principal Diagonal win!",
        principal_diagonal_sum<secondary_diagonal_sum: "Secondary Diagonal win!"
    }.get(True, 'Draw!')