...
def solve(a, b):
    challenge_points = list((1,0) if point_a>point_b else (0,1) if point_a<point_b else (0,0) for point_a,point_b in zip(a,b)) 
    alice = sum(p[0] for p in challenge_points)
    bob = sum(k[1] for k in challenge_points)
    return {
        alice>bob: f'{alice}, {bob}: Alice made "Kurt" proud!',
        bob>alice: f'{alice}, {bob}: Bob made "Jeff" proud!'}.get(True, f'{alice}, {bob}: that looks like a "draw"! Rock on!')