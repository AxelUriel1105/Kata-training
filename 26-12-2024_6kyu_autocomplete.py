...
def autocomplete(input_, dictionary):
    input_ = ''.join(l for l in input_ if l.isalpha())
    filtered_dict = [w for w in dictionary if w.lower().startswith(input_)]
    return filtered_dict if len(filtered_dict)<= 5 else filtered_dict[:5]